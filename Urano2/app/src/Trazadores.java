...
class Trazadores{
    public static String[][] trazadoresLineales(double[] x, double[] y) throws IOException{
        ...
        File file = new File("trazadoresLineales.txt");
        ...
        //ab = solucion el sistema A,b
        double[] ab = SistemaDeEcuaciones.eliminacionGaussianaTotal(A,b);
        ...
        writer.write("Funciones interpolantes + \n");
        l = 0;
        for(int i=0; i< ab.length; i+=2){
            ...
            writer.write(polinomio + "     " + intervalo + "\n");
            l += 1;
        }
        writer.close();
        return polinomios;
    ...
    public static String[][] trazadoresCuadraticos(double[] x, double[] y) throws IOException{
        ...
        String[][] polinomios = new String[n][n];
        String polinomio, intervalo;
        File file = new File("trazadoresCuadraticos.txt");
        FileWriter writer = new FileWriter(file);
        int j,l;
        for(int i=0; i<n; i++){
            j = 2*i;
        ...
        A[(3*n)-1][0] = 1;
        double[] abc = SistemaDeEcuaciones.eliminacionGaussianaTotal(A,b);
        writer.write("Matriz A \n");
        for (double[] i : A) {
            for (double Ai : i) {
                writer.write(Ai + " ");
        ...
        for(int i=0; i< abc.length; i+=3){
            ...
            polinomios[l][0] = polinomio;
            writer.write(polinomio);
            intervalo = String.valueOf(x[l]) + "<= x >= " + String.valueOf(x[l+1]);
            polinomios[l][1] = intervalo;
            writer.write("    " + intervalo + "\n");
            l += 1;  
        }
        writer.close();
        return polinomios;
    ...
    String[][] polinomios = new String[n][n];
    String polinomio, intervalo;
    File file = new File("trazadoresCubicos.txt");
    FileWriter writer = new FileWriter(file);
    int j,l;
    for(int i=0; i<n; i++){
        j = 2*i;
